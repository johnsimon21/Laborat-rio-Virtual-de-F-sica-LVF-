...
function drawCuboid(ctx,angle)
{
	...
	var height = 60; // Altura do retângulo

	var nextX = this.pX + Math.cos(angle * Math.PI / 180.0) * width;
	var nextY = this.pY + Math.sin(angle * Math.PI / 180.0) * width;
	ctx.lineTo(nextX, nextY);
	nextX = nextX + Math.sin(angle * Math.PI / 180.0) * height;
	nextY = nextY - Math.cos(angle * Math.PI / 180.0) * height;
	ctx.lineTo(nextX, nextY);
	nextX = nextX - Math.cos(angle * Math.PI / 180.0) * width;
	nextY = nextY - Math.sin(angle * Math.PI / 180.0) * width;
	ctx.lineTo(nextX, nextY);
